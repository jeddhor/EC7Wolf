...
namespace
{
	...
	// The splash art. Looked up by texture name so it can come from anywhere the
	// resource system knows about -- in practice a small archive in the data
	// directory, because the art derives from the commercial splash screen and
	// must not enter the source tree.
	FTexture *Backdrop()
	{
		static FTextureID id = TexMan.CheckForTexture("C7MENUBG", FTexture::TEX_Any);
		if(!id.isValid())
			return NULL;
		return TexMan(id);
	}
